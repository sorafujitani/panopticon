...
func validateResult(result map[string]any, spec StepSpec, state map[string]any) (bool, string) {
	...
	for index, raw := range artifacts {
		...
		if !isWithin(absolute, runDir, worktree) {
			return false, fmt.Sprintf("artifacts[%d].path が run/worktree 外です: %s", index, absolute)
		}
...
func (engine *FlowEngine) waitCustomSubmission(state map[string]any, spec StepSpec, target string) error {
	...
	workingTimeout := spec.TimeoutSec * 1000
	...
	if remaining <= 0 {
		return engine.handleCustomSubmissionTimeout(state, spec, target, &CommandError{Message: fmt.Sprintf("agent %s の settled 待機がタイムアウトしました", target), Argv: []string{engine.Client.Executable, "agent", "wait", target}, Code: "timeout"})
	}
